    ...
    <>
      ...
      <div className={style.navigation}>
        <button
          ...
          aria-label="Previous"
          onClick={() => handleBtn(idx > 0 ? idx - 1 : amt)}
        >
          <GrPrevious />
        </button>
        {nodes.map((n, i) => (
          <button
            className={`${style.dot} ${i === idx ? style.active : ""}`}
            aria-label={`Go to: ${n.frontmatter.title}`}
            onClick={() => handleBtn(i)}
          >
            <Image
              secure="true"
              cloudName="casperleerink"
              publicId={n.frontmatter.image}
        ...
        <button
          className={style.button}
          aria-label="Next"
          onClick={() => handleBtn(idx >= amt ? 0 : idx + 1)}
        >
          <GrNext />
        </button>
      </div>
